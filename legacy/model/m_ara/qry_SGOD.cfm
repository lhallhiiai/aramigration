<!--- Jeff Bryant's Queries for the organization bind  --->
<!--- Cascading set of functions which determines Sector, Group, Operations, Divisions --->
<!--- Get array of sectors --->
    <cffunction name="get_sector" access="remote" returnType="array">
        <cfargument name="orgcode" type="string" required="false">
        <!--- Define variables --->
...
<!--- cfdump var="#get_sector#" format="text" --->
        <!--- Convert results to array --->
        <cfloop index="i" from="1" to="#get_sector.recordcount#">
            <cfset result[i][1]=get_sector.sctr[i]>
            <cfset result[i][2]=get_sector.sctr[i]>
        </cfloop>
	...
	ORDER by dvsn
        ...
        <cfloop index="i" from="1" to="#get_division.recordcount#">
            ...
            <!--- cfset result[i][2]=get_division.dvsn[i] --->
